...
router.post("/api/ai-builder-style", async (req, res) => {
  try {
    const { styleDesign, aiBuilderPageID, sectionID } = req.body;
...
router.get("/api/ai-builder-style", async (req, res) => {
  try {
    const { data: aiBuilderStyle, error: selectError } = await supabase.from("ai_builder_styles").select(`*, ai_builder_pages:ai_builder_pages_id(*), sections:section_id(*)`);
